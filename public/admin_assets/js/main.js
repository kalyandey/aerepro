$(function () {
    ...
    $("input[name='select_awarded_type']").click(function(){
        var getVal = $("input[name='select_awarded_type']:checked").val();
        if (getVal == 'contractor_bidder') {
            $('.contractor_bidder').show();
            $('.add_contractor').hide();
            $('.add_bidder').hide();
        }else if(getVal == 'bidder'){
            $('.contractor_bidder').hide();
            $('.add_contractor').hide();
            $('.add_bidder').show();
        }else if(getVal == 'contractor'){
            $('.contractor_bidder').hide();
            $('.add_contractor').show();
            $('.add_bidder').hide();
        }
    });
    $("input[name='select_awarded_type']:checked").trigger('click');
    //BEGIN MENU SIDEBAR
...
$("#userPasswordErrSpan").css('display','block');
...
/*if (password.match(/([!,%,&,@,#,$,^,*,?,_,~])/)) {
    ...
    $(".signupForm input[type=submit]").attr('data-i',strength);
    if (strength != 3) {
  ...
  $(function () {
         $('#track_datetimepicker').datetimepicker({
           format: 'MM/DD/YYYY',
           autoclose: true,
           pickTime: false,
          })});
