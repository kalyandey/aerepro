...
MultipleImageUploader.prototype.uploadImage = function (file,index,filename) {
        var formData = new FormData();
        ...
        if (uploaderObj.uploadImg.data != undefined) {
            ...
            formData.append('category_id', $(uploaderObj.target).parents('.appendsubbody').find(".append_plan_category").val());
        }
                        ...
                        if (i < uploaderObj.upFiles.length) {
                                 var reader = new FileReader();
                                  ...
                                  reader.onloadend = (function(file) {
                                    return function(evt) {
                                      uploaderObj.uploadImage(evt.target.result,i,file.name)
                                    };
                                  })(uploaderObj.upFiles[i]);
...
MultipleImageUploader.prototype.preview = function() {
   ...
   var anyWindow = window.URL || window.webkitURL;
            ...
            for(var i = 0; i < fileList.length; i++){
              ...
              var loadMoreContent = $(uploaderObj.target).parents('.appendsubbody').find(".loadMorePicContainer")
