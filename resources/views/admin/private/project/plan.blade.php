                            ...
                            <div class="row">
                                ...
                                <input type="hidden" name="project_id" value="{{$details->id}}" />
                                    ...
                                    <br>
                                        ...
                                        <a href="{{URL::route('admin_private_project_speces',[$details->id])}}" class="btn btn-primary" type="button">Submit</a>
                                        <a href="{{URL::route('admin_private_project_speces',[$details->id])}}" class="btn btn-warning" type="button">Skip</a>
                                        <a href="{{URL::route('admin_private_project_details',[$details->id])}}" class="btn btn-default" type="button">Back</a>
                                        </div>
                                        </div>
                                        {!! Form::close() !!} 
                                </div>
            ...
            var fileList = [];
                        ...
                        function removeBox(element) {
                                   if (confirm('Are you sure??')) {
                                                ...
                                                $.ajax({
                                                  url : ADMIN_URL + '/private-project-plans-remove',
                                                  type : 'POST',
                                                  beforeSend:function(){
                       ...
                       function uploadImage(target, file,fileList, index,filename,previewBox) {
                        ...
                        ajax.open("POST", ADMIN_URL+'/private-project-upload-pdf' ,true);
                    ...
                    function removePdf(element) {
                        if (confirm('Are you sure??')) {
                                    ...
                                    $.ajax({
                                      url: ADMIN_URL + '/private-project-plan-delete',
                                       type:'post',
                                       data:{
                                         "_token": CSRF_TOKEN,
                        ...
                        $.ajax({
                           url: ADMIN_URL + '/private-project-plan-lists',
                           type:'post',
                           data:{
                             "_token": CSRF_TOKEN,
                                    ...
                                    if (response.length  > 0) {
                                             $.each(response, function(index, element){
                                                         ...
                                                         .attr('data-item', i );
                                                            ...
                                                            var h_i = $('<img/>').appendTo(h_a).attr('src',objectUrl);
                                                            var h_title = $("<p/>").appendTo(h).html(p.plan_name);
