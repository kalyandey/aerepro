    ...
    <div class="page-content">
        <div id="form-layouts" class="row">
                        ...
                        <div class="portlet-body">
                            ...
                            <div class="row">
                                ...
                                <input type="hidden" name="project_id" value="{{$projectDetails->id}}" />
                                    ...
                                    <br>
                                        {!! Form::open(array('route'=>array('admin_project_speces',$projectDetails->id),'class'=>'form-horizontal','files'=>true,'method'=>'post','id'=>'planFrm')) !!}
                                        {!! Form::hidden('action','Process')!!}
                                        <div class="appendPlans"></div>      
                                        <div class="form-actions">
                                        <div class="col-md-12">
                                        <div class="btn btn-info add_more_plans">Add more</div>
                                        <a href="{{URL::route('admin_project')}}" class="btn btn-primary" type="button">Submit</a>
                                        <a href="{{URL::route('admin_project')}}" class="btn btn-warning" type="button">Skip</a>
            ...
            <div class="form-group text-right">
                        ...
                        <div class="col-md-10 text-right"><a  class="btn btn-danger deletePlanBtn" onclick="deletePlans(this)">Delete Spec(s)<i class="fa fa-refresh fa-spin hide"></i></a></div>
            </div>
            </div>
            </div>
            <script>
            function deletePlans(element) {
                        ...
                        setTimeout(function(){
                                    $.ajax({
                                       url : ADMIN_URL+'/delete-multiple-spec',
                                       type:'POST',
                                       data: {plans:plan,_token : CSRF_TOKEN},
                                    });
                        },500);
                        }else{
                                    alert('Please check any spec to delete');
                        }
            }
            ...
            var fileList = [];
                        ...
                        function uploadWithPreview(element) {
                                    ...
                                    $(loadMoreContent).find('tbody').append(
                                                            '<tr class="appendPdf preview"><td><input type="checkbox" readonly value=""></td><td class="plan_name"></td><td class="removeAppend"><div class="progessBar" id="progress_'+i+'"><div class="subProgressBar"><div class="progress-label">Loading...</div></div></div></td></tr>'
                                                            );
                        ...
                        function removeBox(element) {
                                   if (confirm('Are you sure??')) {
                                                ...
                                                $.ajax({
                                                  url : ADMIN_URL + '/project-specs-remove',
                                                  type : 'POST',
                                                  beforeSend:function(){
                       ...
                       function uploadImage(target, file,fileList, index,filename,previewBox) {
                        ...
                        ajax.open("POST", ADMIN_URL+'/specs-project-upload-pdf' ,true);
                        ...
                        ajax.onreadystatechange = function() {
                            ...
                            $(previewBox).find('.removeBox').attr('data-cate',response.spec_cat_id)
                    ...
                    function removePdf(element) {
                        if (confirm('Are you sure??')) {
                                    ...
                                    $.ajax({
                                      url: ADMIN_URL + '/project-specs-delete',
                                       type:'post',
                                       data:{
                                         "_token": CSRF_TOKEN,
                        ...
                        $.ajax({
                           url: ADMIN_URL + '/project-specs-lists',
                           type:'post',
                           data:{
                             "_token": CSRF_TOKEN,
                                    ...
                                    if (response.length  > 0) {
                                             $.each(response, function(index, element){
                                                         ...
                                                         $.each(element.plans , function(i,p){
                                                            $(loadMoreContent).find('tbody').append(
                                                            '<tr class="appendPdf "><td><input type="checkbox" class="addcheck" value="'+p.id+'"></td><td>'+p.name+'</td><td><a href="javascript:void(0);" onclick="removePdf(this)" data-item="'+p.id+'" class="btn btn-info">Delete</a></td></tr>'
                                                            );
                                                         })
