	...
	<div class="report-table">
	  ...
	  <div class="table_top clear">
	    ...
	    <div class="pagination">
                ...
                @include('front.pagination.custom', ['paginator' => $list->appends(['project_name' => $project_name,'project_no'=>$project_no,'county'=>$county_id,'category'=>$category_id,'contractor_id'=>$contractor_id,'type'=>$type_id,'s'=>$s,'stype'=>$stype,'change_view'=>$pegination])])
	 ...
	 {!! Form::open(array('route'=>array('project-print'),'class'=>'table_chkbox2')) !!} 
	  ...
	  <div class="table_bot clear">
		...
		<thead>
		  ...
		  <tr>		
		    ...
		    @if($s == 'prebid')
			@if($stype == 'DESC')
			      <a href="{{URL::route('planroom_list').'?s=prebid&stype=ASC'}}"><img src="{{asset('images/arrow5.png')}}"></a>
			@endif
			@if($stype == 'ASC')
			      <a href="{{URL::route('planroom_list').'?s=prebid&stype=DESC'}}"><img src="{{asset('images/arrow4.png')}}"></a>
			@endif
		     @else
			<a href="{{URL::route('planroom_list').'?s=prebid&stype=ASC'}}" class="top-arrow"><img src="{{asset('images/arrow1.png')}}"></a>
			<a href="{{URL::route('planroom_list').'?s=prebid&stype=DESC'}}" class="down-arrow"><img src="{{asset('images/arrow3.png')}}"></a>
		     @endif
		    </th>
		...
		<tbody>
			...
			@endif
		       ...
		       @php
                       $prebid_close_date = $bid_close_date = '';
                       @endphp
                       @if($l->pre_bid_meeting_date != '' && $l->pre_bid_meeting_date != '0000-00-00')
		       @php
                       $prebid_close_date = explode('-',$l->pre_bid_meeting_date);
                       $prebid_close_date  = $prebid_close_date[1].'/'.$prebid_close_date[2].'/'.$prebid_close_date[0];
                       @endphp 	
                       
		       @endif

                      <td data-title="Pre-Bid Date" class="bidCloseDate">
		      <em>{!! $prebid_close_date !!}</em>
		      @if($l->mandatory_pre_bidding == 'Yes')
		      <div class="no_start">*<span>Mandatory</span></div>
		      @endif
		      </td>
		       @if($l->bid_close_date != '' && $l->bid_close_date != '0000-00-00')
		       @php
                       $bid_close_date = explode('-',$l->bid_close_date);
                       $bid_close_date  = $bid_close_date[1].'/'.$bid_close_date[2].'/'.$bid_close_date[0];
                       @endphp
		       @endif
		       <td data-title="Bid Date" class="bidCloseDate">{!! $bid_close_date !!}</td>
	  ...
	  <div class="btn-hold">
	    ...
	    <div class="bt-slct-hld">
                ...
                {{Form::open(['id' => 'view_pegi','method'=>'GET'])}}
		{{Form::hidden('project_name',$project_name)}}
		{{Form::hidden('project_no',$project_no)}}
		{{Form::hidden('county',$county_id)}}
		{{Form::hidden('category',$category_id)}}
		{{Form::hidden('type',$type_id)}}
		{{Form::hidden('s',$s)}}
		{{Form::hidden('stype',$stype)}}
