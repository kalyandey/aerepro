	...
	<div class="popup_top clear">
	  ...
	  <div class="pop_top">
	    <ul>
	      ...
	      <li>Crack Sealing - <span>{!! (count($details->county) >0)?$details->county->name:'N/A'; !!}</span></li>
	    </ul>
	  </div>
	</div>
		...
		<div class="tab-box-hold plan">
		    ...
		    <div class="cart_success" style="color:green;display:none"><b>Cart added successfully!</b></div>
			    ...
			    <div class="checkPlanContainer">
                            <h3>{{Form::checkbox('plan_check_cat','','',array('class'=>'plan_check_cat'))}}{!! $k !!}</h3>
                            @foreach($plan as $p)
                                <div class="plan_view">
				...
				@if(Helpers::isFileExist('uploads/project/plan/'.$p->file_name) && $p->file_name != '')
                                <span class="view view_plan_images">View</span>
				@if($p->file_images != '')
				<!--<div class="plan-overlay"></div>-->
				<div class="view-pdf-container" >
				<span class="pdf_close_button"><img src="{{asset('images/close.png')}}" alt="no img"></span>
				  @php
				  $image = explode(',',$p->file_images);
				  @endphp
				  @if(count($image) > 1)
				  <div class="nev-div">
				    <ul>
				      <li class="prev-link">Prev</li>
				      <li class="next-link">Next</li>
				    </ul>
				  </div>
				  @endif
				  ...
				  @foreach($image as $key=>$i)
				    ...
				    <li class="{{($key == 0)?'active':''}}"><img src="{{asset('uploads/project/plan/images/'.$i)}}"> </li>
				    @endif
				  @endforeach
				</ul>
				</div>
