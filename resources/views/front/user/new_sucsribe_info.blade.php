...
@section('content')
	...
	<h3><strong>Payment</strong></h3>
	  <table id="no-more-tables" class="res-table1">
		...
		<tr>
		  ...
		  <td data-title="PRICE"> $
		  @if($subscription_type[$subscription->id] == 'quarterly')
		    {{ $subscription->quarterly_price .' ( Quarterly ) '}}
		    @else
		    {{ $subscription->yearly_price .' ( Yearly )' }}
		  @endif
		  </td>
		</tr>
	      @endforeach
		...
		<td data-title="">{!! '$'.number_format($newsubscriptionFees,2) !!}</td>
	      </tr>
	    </tfoot>
	  </table>
	...
	<div class="form-report clear">
	  ...
	  @endif
	  {!! Form::open(array('route'=>array('new_subscription_payment'),'files'=>true,'class'=>'paymentForm')) !!}
	  {!! Form::hidden('subscription_type',implode(',',$subscription_type)) !!}
	  {!! Form::hidden('subscription_id',implode(',',$subscription_id)) !!}
	    <div class="form-msg">
