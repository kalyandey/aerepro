...
class PrivateuserController extends Controller
{
    ...
    public function lists(Request $request){
        ...
        if($data['key'] !='' || $data['company']!=''){
	     ...
	     $data['lists'] = Private_company::join('private_company_assigns', 'private_companies.id', '=', 'private_company_assigns.user_id')
	        ->join('private_companies as PC','PC.id','private_company_assigns.company_id')
                ->selectRaw('ae_private_companies.status')
		->selectRaw('ae_private_companies.id')
		->selectRaw('ae_private_company_assigns.user_id')
		->selectRaw('ae_private_company_assigns.company_id')
		->selectRaw('ae_private_companies.email')
                ->selectRaw('ae_private_companies.first_name')
                ->selectRaw('ae_private_companies.last_name')->where(function($query) use ($data) {
		if($data['key'] != ''){
		$query->where('private_companies.first_name','LIKE','%'.$data['key'].'%');
		$query->orWhere('private_companies.last_name','LIKE','%'.$data['key'].'%');
		$query->orWhere('private_companies.email','LIKE','%'.$data['key'].'%');
		$query->orWhere(\DB::raw('CONCAT(ae_private_companies.first_name," ",ae_private_companies.last_name)'),'LIKE','%'.$data['key'].'%');
		//$query->orWhere('company_name','LIKE','%'.$data['key'].'%');
		}
		...
		})->where('private_companies.user_type','=','user')->orderBy(\DB::raw('CONCAT(ae_private_companies.first_name, " ", ae_private_companies.last_name)'),'asc')->paginate(10);
		//})->where('private_companies.user_type','=','user')->orderBy('private_companies.company_name','asc')->orderBy('PC.company_name','asc')->paginate(10);
		...
		->selectRaw('ae_private_companies.email')
                ->selectRaw('ae_private_companies.first_name')
                ->selectRaw('ae_private_companies.last_name')
                ->where('private_companies.user_type','=','user')
		->orderBy(\DB::raw('CONCAT(ae_private_companies.first_name, " ", ae_private_companies.last_name)'),'asc')->paginate(10);
		
               // ->orderBy('PC.company_name','asc')->paginate(10);
